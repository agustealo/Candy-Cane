=== Candy Cane ===
Contributors: agustealo
Tags: responsive-layout, custom-menu, featured-images, threaded-comments
Requires PHP: 7.4
License: GPLv2
License URI: https://www.gnu.org/licenses/gpl-2.0.html

Candy Cane is a responsive classic WordPress theme for portfolios, creative work, blogs, and magazine-style sites.

== Description ==

Candy Cane preserves its original classic-theme layout and visual identity while its WordPress integration is modernized for current releases.

Existing installations keep the established menu locations, widget areas, image-size identifiers, classic PHP templates, and Foundation-era grid classes. Updating the theme does not require rebuilding site content or converting the site to a block theme.

== Installation ==

1. Install Candy Cane in the WordPress themes directory or upload the theme ZIP through WordPress.
2. Activate Candy Cane under Appearance > Themes.
3. Assign the Main Navigation and Secondary Navigation menu locations.
4. Configure the Right Sidebar and Footer 1 through Footer 4 widget areas as needed.
5. Publish posts and pages normally. Featured images are used by the existing Candy Cane post-grid presentation.

== Compatibility ==

The maintained repository quality gate checks PHP syntax from PHP 7.4 through PHP 8.5, WordPress Coding Standards, PHPCompatibilityWP, a fresh WordPress 7.1.1 runtime, browser-rendered visual continuity against the pre-modernization Candy Cane theme across desktop, tablet, and compact layouts, and a clean installation of the generated consumer ZIP into WordPress.

The release-package gate verifies the installable archive contents, repeatable SHA-256 output for the same Git ref, packaged theme metadata and runtime files, activation from the ZIP, preserved menu/sidebar/image-size contracts, rendered content, public stylesheets, and fatal-error-free execution.

The theme intentionally remains a classic WordPress theme. It is not a Full Site Editing conversion and does not require a content migration.

== License ==

Candy Cane is licensed under the GNU General Public License v2.0.
